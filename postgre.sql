...
CREATE TABLE elmasri.funcionario (
				...
				-- a FK declara autorrelação da tabela
				...
				-- definição da chave primária (cpf)
);
...
CREATE TABLE elmasri.departamento (
				...
				-- FK relacionada à PK da tabela funcionários
                data_inicio_gerente DATE,
                CONSTRAINT pk_departamento PRIMARY KEY (numero_departamento)
				-- criação de uma nova PK
);
...
-- criação da chave única (UK) ou chave alternativa (AK)
...
COMMENT ON COLUMN elmasri.projeto.local IS 'Localização do projeto.';

-- criação de mais uma AK
CREATE UNIQUE INDEX projeto_idx
 ON elmasri.projeto
 ( nome_projeto );

-- criando a tabela das localizações de departamentos
CREATE TABLE elmasri.localizacoes_departamento (
                numero_departamento INTEGER NOT NULL,
                local VARCHAR(15) NOT NULL,
                CONSTRAINT pk_localizacoes_departamento PRIMARY KEY (numero_departamento, local)
				-- definição de duas PK, uma nova e outra que se relaciona com a tabela de departamentos
);
...
-- criando tabela de trabalhos em que os funcionários estão
CREATE TABLE elmasri.trabalha_em (
                cpf_funcionario CHAR(11) NOT NULL,
                numero_projeto INTEGER NOT NULL,
                horas NUMERIC(3,1),
                CONSTRAINT pk_trabalha_em PRIMARY KEY (cpf_funcionario, numero_projeto)
				-- definição de duas PK, que se relacionam com as tabelas de funcionários e de projetos
);
...
CREATE TABLE elmasri.dependente (
				...
				-- definição de duas Pk, uma que se relaciona com a tabela de funcionários e outra para diferenciar os dependentes entre si, formando uma super chave
);
...
-- relações entre as tabelas abaixo:
...
VALUES	('João', 'B', 'Silva', '12345678966', '1965-01-09', 'Rua das Flores, 751, São Paulo, SP', 'M', '30000', '33344555587', 5),
		('Fernando', 'T', 'Wong', '33344555587', '955-12-08', 'Rua da Lapa 34, São Paulo, SP', 'M', '40000', '88866555576', 5),
